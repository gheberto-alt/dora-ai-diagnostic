...
import { 
  ...
  Sparkles, Award, BarChart3, AlertTriangle, Zap, ChevronRight, UserCheck, Cloud, Wifi
} from 'lucide-react';
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'pmi-dora-vsm-2026';
...
export default function App() {
  ...
  useEffect(() => {
    if (!db || !user) {
      // Offline / Local fallback dataset generation if database is not active
      const localStored = localStorage.getItem('dora_vsm_responses');
      if (localStored) {
        try { setAttendeesResponses(JSON.parse(localStored)); } catch (e) {}
      } else if (attendeesResponses.length === 0) {
    ...
    // Mandatory path rule for public collection
    const responsesRef = collection(db, 'artifacts', appId, 'public', 'data', 'dora_responses');
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setAttendeesResponses(cloudData);
      },
      (error) => {
  ...
  const generateInitialSimulatedData = () => {
    const sampleNames = ['María González', 'Carlos Rodríguez', 'Ana Silva', 'Diego Morales', 'Laura Pérez', 'Felipe Castro', 'Sofia Gomez', 'Javier Fernandez'];
    const initialData = [];
    ...
    for (let i = 0; i < 35; i++) {
      const qAnswers = {
        ...
        5: Math.floor(Math.random() * 4) + 2,
        6: Math.floor(Math.random() * 3) + 1,
        7: Math.floor(Math.random() * 3) + 2
      };
    ...
    setAttendeesResponses(initialData);
  };
    ...
    attendeesResponses.forEach(resp => {
      if (resp.answers) {
        Object.entries(resp.answers).forEach(([qId, val]) => {
          categorySums[qId] = (categorySums[qId] || 0) + Number(val);
          totalSum += Number(val);
        });
      }
    });
  ...
  const handleAttendeeSubmit = async (e) => {
    ...
    try {
      if (db && user) {
        ...
        const localList = [...attendeesResponses, { ...newResponse, id: 'local_' + Date.now() }];
        setAttendeesResponses(localList);
        localStorage.setItem('dora_vsm_responses', JSON.stringify(localList));
      }
      ...
      console.error("Error saving response to cloud:", err);
    } finally {
      setSubmitting(false);
    }
  };
                ...
                <span className="text-xs text-slate-400 flex items-center gap-1">
                  {isCloudConnected ? (
                    <>
                      ...
                      <span className="text-emerald-400 font-medium">Nube Sincronizada</span>
                    </>
                  ) : (
                    <>
                      <Wifi className="w-3 h-3 text-amber-400 inline" />
                      <span>Modo Local / Demo</span>
                    </>
                  )}
                </span>
              </div>
              <h1 className="text-base font-bold text-white tracking-tight">Flash VSM: Diagnóstico DORA AI 2025</h1>
            </div>
          </div>
            ...
            {!submittedResult ? (
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      {currentQuestionStep < QUESTIONS.length - 1 ? (
                        ...
                        <button
                          ...
                          className="px-6 py-2.5 text-xs font-bold rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 text-slate-950 hover:from-cyan-400 hover:to-blue-500 shadow-lg shadow-cyan-500/20 flex items-center gap-2 disabled:opacity-50"
                        >
                          {submitting ? 'Enviando a la Nube...' : 'Enviar Diagnóstico en Vivo'}
                        </button>
                      )}
                    </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-400">Gracias, <span className="text-white font-semibold">{submittedResult.name}</span>. Tus respuestas se sincronizaron con la pantalla principal.</p>
                </div>
        ...
        {currentMode === 'speaker' && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 flex flex-wrap items-center justify-between gap-4">
              ...
              {/* Status Badge */}
              <div className="flex items-center gap-2 text-xs bg-slate-950 px-3 py-2 rounded-xl border border-slate-800 text-slate-400">
                <Cloud className="w-4 h-4 text-cyan-400" />
                <span>Base de datos: <strong className="text-white">{isCloudConnected ? 'Firebase Conectado' : 'Modo Demostración'}</strong></span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Proyección acumulada en tiempo real de los asistentes en sala.
                  </p>
                </div>
              ...
              {/* RIGHT COLUMN: DIAGNOSIS & SPEAKER STRATEGIC GUIDELINES */}
        ...
        {currentMode === 'qr' && (
          <div className="max-w-xl mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center space-y-6 shadow-2xl my-6">
            ...
            <div className="space-y-1 bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs text-slate-300 font-mono">
              <div>Escanea o ingresa desde tu móvil</div>
            </div>
